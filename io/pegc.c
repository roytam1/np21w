...
//   SL9821 技術的なはなし - PEGC https://www.satotomi.com/sl9821/sl9821_tec5.html
...
static void pegc_shift_push(UINT8 value) {
	...
	if (count >= PEGC_SHIFTBUF_SIZE) {
		// PEGC_SHIFTBUF_SIZEを超えたら一番古いものを破棄
...
static int pegc_plane_base_address(UINT32 address, UINT32 *addr) {
	...
	// 1画面モードならA8000h～B0000h～
	// 2画面モードならA8000hのみ有効、GDC I/O A6hで描画ページを選択
