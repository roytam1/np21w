							...
							if (hBmp) {
								...
								if (!hasError) {
									...
									if (fGet) {
										...
										//if (tgtPBmp.bmWidth == 55 && tgtPBmp.bmHeight == 18) {
										//	static int yyyy = 0;
										...
										//	//BitBlt(npdispwin.hdcBltBuf, 0, yyyy, biHeader.biWidth, height, hdc, 0, 0, SRCCOPY);
										//	yyyy += 24;
						...
						if (pBitsBase) {
							...
							if (npdisp.longjmpnum == 0) {
								...
								if (height > iScanHost) {
									...
									if (height == 6 && biHeader.biWidth == 128 && biHeader.biBitCount == 1) {
										height = height;
									}
