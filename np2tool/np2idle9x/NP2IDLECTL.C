/* NP2IDLECTL.C
 *
 * Win32 command-line controller for NP2IDLE.VXD.
 * Intended for Windows 9x.  Uses only ordinary Win32 APIs and can be
 * built with Visual C++ 6.0.  /MT is recommended so the EXE does not depend
 * on a separately installed C runtime DLL.
 */
...
static HANDLE open_np2idle(void)
{
    ...
    /* The Win95 VxD interface accepts zero for the creation
